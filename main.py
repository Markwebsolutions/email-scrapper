...
async def stream_logs(cmd):
    try:
        ...
        if process.returncode != 0:
            print(f"Command {cmd} exited with {process.returncode}")
...
async def run_fb_scraper():
    ...
    if not os.path.exists(SERVICE_JSON):
        return {"status": "error", "message": "service_account.json missing"}

    script = f"{BASE_DIR}/node_scripts/facebook_scraper.js"
    if not os.path.exists(script):
        return {"status": "error", "message": "FB script missing"}
